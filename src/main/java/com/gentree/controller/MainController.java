    ...
    public static void createDotGraph(String dotExport,String filepath,String format) throws IOException
    {
        ...
        switch (format)
        {
            ...
            case "png":
                ...
                break;
            case "jpeg":
                BufferedImage temp = new BufferedImage( Graphviz.fromString(dotExport).render(Format.PNG).toImage().getWidth(), Graphviz.fromString(dotExport).render(Format.PNG).toImage().getHeight(), BufferedImage.TYPE_INT_RGB);
                temp.createGraphics().drawImage( Graphviz.fromString(dotExport).render(Format.PNG).toImage(), 0, 0, Color.BLACK, null);
